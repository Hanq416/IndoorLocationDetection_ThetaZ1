...
function Ipano = imgstiching(Idf)
...
IR = imresize(IR,[size(IL,1), size(IL,2)]);
close figure 1;
%camera parameters retrived from multiple attempt, can be improved with
%futher works
%{
fovL  = 183; rollL = 90; tiltL = 0; panL = -2.5; %paper usage, for better view
fovR  = 185; rollR = 90-0.3; tiltR = -0.5; panR = 180;
%}

fovL  = 183; rollL = 0; tiltL = 0; panL = -2.5; 
fovR  = 185; rollR = -0.3; tiltR = -0.5; panR = 180;

EL = imfish2equ(IL,fovL,rollL,tiltL,panL); ER = imfish2equ(IR,fovR,rollR,tiltR,panR);
...
ELR = alpha.*double(EL) + (1-alpha).*double(ER); Ipano = uint8(ELR);
end

function [IE2,mask] = trimImageByFov(IE,fov,pan)
w  = int16(size(IE,2)); we = w*(fov/360)/2; ce = mod(w*(0.5+pan/360),w);
